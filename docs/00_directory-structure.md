...
│   ├── main.ts            # クエリ層の統合エクスポート
│   ├── areaQuery.ts       # エリアデータ取得
│   ├── searchQuery.ts     # 検索クエリ
│   ├── stationQuery.ts    # 駅データ取得
│   └── transformers.ts    # DB レコード → フロントエンド用オブジェクト変換
│
└── seed/
    ├── 00_master/     # エリア・駅・路線・鉄道会社の基盤マスタ
    ├── chains/        # チェーン店（Starbucks / Doutor 等）の SQL データ
    └── add_tables/    # スキーマ拡張の履歴（Areas, Stations など）
~~~
